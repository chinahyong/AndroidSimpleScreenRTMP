...
public class ScreenRecorderService extends Service {
	...
	public static final int DEFAULT_VIDEO_BITRATE = 1024 * 1500;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		// System dpi
		mSelectedVideoDpi = intent.getIntExtra(EXTRA_SCREEN_DPI, getApplicationContext().getResources().getDisplayMetrics().densityDpi);
	...
	private void startRecording() {
		...
		if (result == -1) {
			mRTMPMuxer = null;
			Toast.makeText(getApplicationContext(), "RTMP address is error", Toast.LENGTH_SHORT).show();
			return;
		}
		// 视频编码失败
		if (!prepareVideoEncoder()) {
			Toast.makeText(getApplicationContext(), "Video encode Error", Toast.LENGTH_SHORT).show();
			return;
		}
		// 音频编码失败
		if (!prepareAudioEncoder()) {
			Toast.makeText(getApplicationContext(), "Audio encode Error", Toast.LENGTH_SHORT).show();
			return;
		}
		// 开辟线程用于推流
